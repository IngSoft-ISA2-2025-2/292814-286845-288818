Feature: Crear reservas
    ...
    Scenario: Usuario sin email intenta reservar medicamentos
        Given un email para reserva ""
        And un secret para reserva ""
    ...
    Scenario: Usuario con email y secret válidos reserva en farmacia inválida
        Given un email para reserva "usuario@test.com"
        And un secret para reserva "secret123"
        And un medicamento "Aspirina" con cantidad de 1 unidad
    ...
    Scenario: Usuario reserva un medicamento que requiere prescripción con éxito
        Given un email para reserva "usuario@test.com"
        And un secret para reserva "secret123"
        ...
        And muestra un mensaje de reserva que dice "Reserva creada exitosamente, el medicamento Aspirina requiere receta médica. Por favor, preséntela en la farmacia para validar la reserva."
    ...
    Scenario: Usuario reserva un medicamento que no requiere prescripción con éxito
        ...
        Then el sistema crea la reserva con un estado Pendiente
        And descuenta la unidad del stock de los medicamentos
        And muestra un mensaje de reserva que dice "Reserva creada exitosamente"
    ...
    Scenario: Usuario con email existente pero secret incorrecto intenta reservar
        Given un email para reserva "usuario@test.com"
        And un secret para reserva "secretIncorrecto"
        And el email para reserva "usuario@test.com" ya tiene reservas con secret "secret123"
        And un medicamento "Aspirina" con cantidad de 1 unidad
        And una farmacia "Farmashop"
        When hace click en el botón de reservar
